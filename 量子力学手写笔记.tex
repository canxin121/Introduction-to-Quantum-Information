...
\begin{align*}
  ...
  \int_{-\infty}^{\infty} x^2 e^{-a x^2}\,dx &= \sqrt{\frac{\pi}{4a^3}},\quad a>0.
\end{align*}
指数型积分（Gamma 函数特例）：
\[
...
\[
\begin{aligned}
  [L_x,L_y]&=i\hbar L_z, & [L_y,L_x]&=-i\hbar L_z,\\
  [L_y,L_z]&=i\hbar L_x, & [L_z,L_y]&=-i\hbar L_x,\\
  [L_z,L_x]&=i\hbar L_y, & [L_x,L_z]&=-i\hbar L_y.
\end{aligned}
\]
...
\subsection{矩阵元（常见例子）}
\textbf{一维无限深势阱（能量表象）：} $u_n(x)=\sqrt{\frac{2}{a}}\sin\left(\frac{n\pi x}{a}\right)$。
\[
x_{mn}=\langle m|\hat x|n\rangle
=\int_0^a u_m^*(x)\,x\,u_n(x)\,dx
=
\begin{cases}
\dfrac{a}{2}, & m=n,\\[6pt]
\dfrac{4nma}{\bigl[(m^{2}-n^{2})\pi\bigr]^2}\left[(-1)^{m-n}-1\right], & m\ne n.
\end{cases}
\]
\[
p_{mn}=\langle m|\hat p|n\rangle
=\int_0^a u_m^*(x)\left(-i\hbar\frac{d}{dx}\right)u_n(x)\,dx
=
\begin{cases}
0, & m=n,\\[6pt]
\dfrac{2nm\hbar i}{(m^{2}-n^{2})a}\left[(-1)^{m-n}-1\right], & m\ne n.
\end{cases}
\]

\textbf{动量表象角动量矩阵元：}动量表象下
\[
\hat L_x^{(p)}=i\hbar\left(p_z\frac{\partial}{\partial p_y}-p_y\frac{\partial}{\partial p_z}\right),
\]
\[
\langle \vec p_1|\hat L_x|\vec p_2\rangle
=\hat L_x^{(p_1)}\delta(\vec p_1-\vec p_2)
= i\hbar\left(p_{1z}\frac{\partial}{\partial p_{1y}}-p_{1y}\frac{\partial}{\partial p_{1z}}\right)
...
能量期望值：
\[
  \langle H \rangle = \langle E \rangle = \sum_n |C_n|^2 E_n.
\]
...
\[
  ...
  \langle T\rangle=\frac{\hbar^2}{2m a_0^2}.
\]
